...
> Let a project holding an installed flavor find out that its copy no longer matches the contract, so an edited or drifted flavor fails loudly instead of silently skipping whichever phase reads the missing section.
...
- [ ] Document it in `docs/flavors.md`, and hand BR-005's **Validated by** revision to `qa`
